...
/** Filesystem paths for SSR/prerender — must match scripts/content-config.cjs */
const SSR_CONTENT_ROOT = 'submodules/ByByteLessons/content';
const SSR_INDEX_PATH = 'src/generated/content/index.json';
...
export class LessonsService {
  ...
  loadLessonsIndex(): Observable<LessonIndex> {
    ...
    const indexUrl = `${environment.contentBasePath}/index.json`;
    const emptyIndex: LessonIndex = {
      lessons: [],
      platforms: [],
      levels: [],
      tags: [],
    ...
    if (!isPlatformBrowser(this.platformId)) {
      try {
        ...
        const filePath = path.join(process.cwd(), SSR_INDEX_PATH);
        const content = fs.readFileSync(filePath, 'utf8');
        const index = JSON.parse(content) as LessonIndex;
        ...
        return of(emptyIndex);
      }
    }

    return this._http.get<LessonIndex>(indexUrl).pipe(
      tap((index) => {
        this._lessonsIndex.set(index);
        this._isLoading.set(false);
      }),
